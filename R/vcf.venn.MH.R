...
#' I changed line 40 rownames and TabixFile ok fine good
vcf.venn_MH <- function(vcf.files, genome, sample.names = NULL, ...)
{
  ...
  x <- lapply(vcf.files, function(f) rownames((rowData(read.vcf(TabixFile(f), genome, ...)))))
